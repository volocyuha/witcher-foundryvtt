...
export class EntitySheetHelper {
      ...
      if ( el.classList.contains("attribute-key") ) {
        ...
        for ( let i = 0; i < groups.length; i++ ) {
          if (groups[i].value == val) {
            ui.notifications.error(game.i18n.localize("WITCHER.NotifyAttrDuplicate") + ` (${val})`);
            el.value = oldVal;
            attrError = true;
            break;
          }
  ...
  static onAttributeRoll(event) {
    ...
    const shorthand = game.settings.get("wtrpg", "macroShorthand");
  ...
  static validateGroup(groupName, entity) {
    ...
    if ( groups.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("WITCHER.NotifyGroupDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for group keys that match attribute keys.
    if ( attributes.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("WITCHER.NotifyGroupAttrDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for whitespace or periods.
    if ( groupName.match(/[\s|\.]/i) ) {
      ui.notifications.error(game.i18n.localize("WITCHER.NotifyGroupAlphanumeric"));
      return false;
    }
  ...
  static async deleteAttributeGroup(event, app) {
    ...
    new Dialog({
      title: game.i18n.localize("WITCHER.DeleteGroup"),
      content: `${game.i18n.localize("WITCHER.DeleteGroupContent")} <strong>${group.val()}</strong>`,
      buttons: {
        confirm: {
