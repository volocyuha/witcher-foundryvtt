 ...
 * Get an existing wtrpg macro if one exists, otherwise create a new one.
...
export async function createWorldbuildingMacro(data, slot) {
  ...
  const command = `game.wtrpg.rollAttrMacro("${item.label}", "${item.roll}");`;
